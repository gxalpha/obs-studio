...
namespace {
// std::chrono would be so nice. Alas, that's for when we have C++20.
...
QString format_relative_time(time_t from)
{
	time_t now = time(0);
...
void OBSSceneCollections::refreshList()
{
	...
	for (auto collectionRef : main->GetSceneCollectionsSorted()) {
		...
		const QString description = isCurrentCollection
						    ? QTStr("LastUsed.CurrentlyActive")
						    : format_relative_time(0 /*TODO: collection.getLastUsed()*/);
...
void OBSSceneCollections::on_buttonNew_pressed()
{
	...
	try {
		...
		OBSBasic::Get()->SetupNewSceneCollection(result.promptValue);
