...
PropertiesView::PropertiesView(GetProperties getProperties, GetSettings getSettings, QWidget *parent)
	: VScrollArea(parent),
	  getProperties(getProperties),
	  getSettings(getSettings),
	  properties(nullptr, obs_properties_destroy),
	  settings(nullptr, obs_data_release),
	  defaults(nullptr, obs_data_release),
...
namespace {
template<typename T> T getPropertyValue(obs_property_t *property, obs_data_t *settings, obs_data_t *defaults)
{
...
{
	const bool value = getPropertyValue<bool>(prop, settings.get(), defaults.get());
	Row *row = new Row();
...
Row *PropertiesView::createPropertyInt(obs_property_t *prop)
{
	...
	const int value = getPropertyValue<int>(prop, settings.get(), defaults.get());
...
Row *PropertiesView::createPropertyDouble(obs_property_t *prop)
{
	...
	const char *suffix = obs_property_int_suffix(prop);
	const double value = getPropertyValue<double>(prop, settings.get(), defaults.get());
...
Row *PropertiesView::createPropertyText(obs_property_t *prop)
{
	...
	const char *value = getPropertyValue<const char *>(prop, settings.get(), defaults.get());
	switch (type) {
...
Row *PropertiesView::createPropertyList(obs_property_t *prop)
{
	...
	if (type == OBS_COMBO_TYPE_RADIO) {
		blog(LOG_INFO, "Combo list type not yet supported"); // TODO: add.
		return createPropertyNull(prop);
	}
...
CollapsibleRow *PropertiesView::createPropertyGroup(obs_property_t *prop)
{
	...
	if (type == OBS_GROUP_CHECKABLE) {
		...
		container->setChecked(getPropertyValue<bool>(prop, settings.get(), defaults.get()));
...
void PropertiesView::updateProperties(bool reset)
{
	...
	if (reset) {
		...
		defaults.reset(obs_data_get_defaults(settings.get()));
	}
	...
	obs_data_apply(copy, settings.get());
	settings.reset(copy);
